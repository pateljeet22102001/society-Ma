...
export async function createEventAction(input: unknown): Promise<Result> {
  ...
  try {
    const supabase = await createClient(); const { society, user } = await requireSocietyRole(FINANCE_ROLES);
    const { error } = await supabase.from("events").insert({ ...parsed.data, start_date: parsed.data.start_date || null, end_date: parsed.data.end_date || null, due_date: parsed.data.due_date || null, description: parsed.data.description || null, society_id: society.id, created_by: user.id });
...
export async function addEventPaymentAction(input: unknown, confirmed = false): Promise<Result> {
  ...
  try {
    ...
    const { error: payError } = await supabase.from("event_flat_payments").insert({ event_id: row.event_id, society_id: society.id, flat_id: row.flat_id, ...parsed.data, reference_number: parsed.data.reference_number || null, created_by: user.id });
    if (payError) throw payError;
    const paid = Number(row.paid_amount) + parsed.data.amount;
    const { error: updateError } = await supabase.from("event_flat_contributions").update({ paid_amount: paid, pending_amount: Math.max(Number(row.amount) - paid, 0), payment_date: parsed.data.payment_date, status: status(Number(row.amount), paid, row.due_date) }).eq("id", row.id).eq("society_id", society.id);
...
export async function undoEventPaymentAction(contributionId: string): Promise<Result> {
  try {
    const supabase = await createClient(); const { society } = await requireSocietyRole(DELETE_FINANCE_ROLES);
    ...
    const { data: payment } = await supabase.from("event_flat_payments").select("*").eq("contribution_id", row.id).eq("society_id", society.id).order("created_at", { ascending: false }).limit(1).maybeSingle();
    if (!payment) return { success: false, message: "No payment found to undo" };
    const { error: deleteError } = await supabase.from("event_flat_payments").delete().eq("id", payment.id).eq("society_id", society.id); if (deleteError) throw deleteError;
    const { data: remaining } = await supabase.from("event_flat_payments").select("amount,payment_date,created_at").eq("contribution_id", row.id).eq("society_id", society.id).order("created_at", { ascending: false });
    const paid = (remaining || []).reduce((sum, p) => sum + Number(p.amount), 0);
    const { error } = await supabase.from("event_flat_contributions").update({ paid_amount: paid, pending_amount: Math.max(Number(row.amount) - paid, 0), payment_date: remaining?.[0]?.payment_date ?? null, status: status(Number(row.amount), paid, row.due_date) }).eq("id", row.id).eq("society_id", society.id);
...
export async function addEventAavakAction(input: unknown, confirmed = false): Promise<Result> {
  ...
  try {
    const supabase = await createClient(); const { society, user } = await requireSocietyRole(FINANCE_ROLES); const d = parsed.data;
    const warnings: string[] = [];
...
export async function addEventExpenseAction(input: unknown, confirmed = false): Promise<Result> {
  ...
  try {
    ...
    if (warnings.length && !confirmed) return { success: false, requiresConfirmation: true, message: `${warnings.join(" ")} Continue anyway?` };
    const { error } = await supabase.from("event_expenses").insert({ ...parsed.data, society_id: society.id, created_by: user.id });
...
export async function deleteEventRecordAction(table: "event_contributions" | "event_expenses", id: string): Promise<Result> {
  try { const supabase = await createClient(); const { society } = await requireSocietyRole(DELETE_FINANCE_ROLES); const { error } = await supabase.from(table).delete().eq("id", id).eq("society_id", society.id); if (error) throw error; refresh(); return { success: true, message: "Record deleted" }; }
